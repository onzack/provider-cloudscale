...
import (
	...
	apisCluster "github.com/onzack/provider-cloudscale/apis/cluster"
	apisNamespaced "github.com/onzack/provider-cloudscale/apis/namespaced"
	"github.com/onzack/provider-cloudscale/config"
	"github.com/onzack/provider-cloudscale/internal/clients"
	controllerCluster "github.com/onzack/provider-cloudscale/internal/controller/cluster"
	controllerNamespaced "github.com/onzack/provider-cloudscale/internal/controller/namespaced"
	"github.com/onzack/provider-cloudscale/internal/features"
	"github.com/onzack/provider-cloudscale/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Cloudscale").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-cloudscale"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-cloudscale",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped Cloudscale APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced Cloudscale APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped Cloudscale controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced Cloudscale controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped Cloudscale controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced Cloudscale controllers")
	}

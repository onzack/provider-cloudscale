...
func Configure(p *config.Provider) {
	p.AddResourceConfigurator("cloudscale_server", func(r *config.Resource) {
		...
		// as an input. And by defining it as a reference to cloudscale.com/v1alpha1.CustomImage
		// object, we can build cross resource referencing. See
		// imageUuidRef in the example in the Testing section below.
		r.References["image_uuid"] = config.Reference{
			Type: "github.com/onzack/provider-cloudscale/apis/cluster/compute/v1alpha1.CustomImage",
		}
	})
}

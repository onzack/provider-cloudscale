...
func Configure(p *config.Provider) {
	p.AddResourceConfigurator("cloudscale_load_balancer_pool_member", func(r *config.Resource) {
		...
		// as an input. And by defining it as a reference to github.com/onzack/provider-cloudscale/apis/namespaced/compute/v1alpha1.Server
		// object, we can build cross resource referencing. See
		// serverRef in the example in the Testing section below.
		r.References["server"] = config.Reference{
			Type: "github.com/onzack/provider-cloudscale/apis/namespaced/compute/v1alpha1.Server",
		}
	})
}

package loadbalancerhealthmonitor

import "github.com/crossplane/upjet/v2/pkg/config"

// Configure configures individual resources by adding custom ResourceConfigurators.
func Configure(p *config.Provider) {
	p.AddResourceConfigurator("cloudscale_load_balancer_health_monitor", func(r *config.Resource) {
		r.ShortGroup = ""
		// We need to override the default group that upjet generated for
		// this resource, which would be "cloudscale"
		r.Kind = "LoadBalancerHealthMonitor"

		// This resource need the load balancer in which health monitor would be created
		...
		// as an input. And by defining it as a reference to github.com/onzack/provider-cloudscale/apis/namespaced/cloudscale/v1alpha1.LoadBalancerPool
		// object, we can build cross resource referencing. See
		// poolUuidRef in the example in the Testing section below.
		r.References["pool_uuid"] = config.Reference{
			Type: "github.com/onzack/provider-cloudscale/apis/namespaced/cloudscale/v1alpha1.LoadBalancerPool",
		}
	})
}

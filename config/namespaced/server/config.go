...
func Configure(p *config.Provider) {
	p.AddResourceConfigurator("cloudscale_server", func(r *config.Resource) {
		...
		// as an input. And by defining it as a reference to github.com/onzack/provider-cloudscale/apis/namespaced/compute/v1alpha1.ServerGroup
		// object, we can build cross resource referencing. See
		// serverGroupRef in the example in the Testing section below.
		r.References["server_group"] = config.Reference{
			Type: "github.com/onzack/provider-cloudscale/apis/namespaced/compute/v1alpha1.ServerGroup",
		}
